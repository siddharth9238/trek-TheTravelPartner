...
const SERVER_PUBLIC_DIR = path.join(REPO_ROOT, 'server', 'public');
export const PUBLIC_DIR = fs.existsSync(path.join(CLIENT_DIST_DIR, 'index.html')) ? CLIENT_DIST_DIR : SERVER_PUBLIC_DIR;

function shouldServeBuiltClient(nodeEnv: string | undefined): boolean {
  const env = nodeEnv?.toLowerCase();
  return env === 'production' || env === 'development' || !env;
}
...
export function applyPlatformSpa(app: express.Application): void {
  ...
  if (!shouldServeBuiltClient(process.env.NODE_ENV)) return;
