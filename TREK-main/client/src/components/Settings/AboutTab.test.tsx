...
describe('AboutTab', () => {
  ...
  it('FE-COMP-ABOUT-005: displays Portfolio link with correct href', () => {
    render(<AboutTab appVersion="2.9.10" />);
    const link = screen.getByText('Portfolio').closest('a');
    expect(link).toHaveAttribute('href', 'https://siddharthsingh-main.vercel.app/');
  });
  ...
  it('FE-COMP-ABOUT-014: Portfolio link hover changes border and box-shadow styles', () => {
    render(<AboutTab appVersion="1.0.0" />);
    const link = screen.getByText('Portfolio').closest('a') as HTMLAnchorElement;
    fireEvent.mouseEnter(link);
